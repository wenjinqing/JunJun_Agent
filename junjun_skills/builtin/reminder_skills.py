...
def set_reminder(content: str, time_spec: str, user_id: str) -> str:
    ...
    条件式提醒（"如果明天下雨就提醒我带伞"）也用本工具而不是发即时消息：
    time_spec 给合理默认（如"明天早上8点"），条件原样写进 content
    （"如果今天下雨，出门带伞"）——即时 send_message 起不到提醒作用
    （2026-08-15 eval 实锤：规划器填不出时间就拿 send_message 顶替）。
    Args:
        content: 提醒内容，如"开会"
