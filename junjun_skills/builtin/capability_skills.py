...
# 还白送攻击面）。模型系列与框架名是公开事实、必须写准——不写准模型就会
# 现场编（2026-08-15 生产实锤：被问「基于什么」时臆造「混元大模型」，
# 框架答「不清楚」）。要改就改这里，别让模型现场编。
_INTRO_TECH = (
    "技术栈：Python 写的；agent 决策内核跑在 LangChain + LangGraph 上"
    "（工具调用循环 + 复杂任务编排成步骤图执行）；QQ 接入走 NapCat（OneBot 协议）；"
    "对话大脑主力是 DeepSeek 系列国产大模型，按槽位分工（聊天/规划/识图各走各的）；"
    "记忆存在本地 SQLite；跑代码有独立的 Docker 沙箱（隔离、无网络）。"
)
...
def introduce_self() -> str:
    ...
    （可以换成你的口气说），不许略过、不许说「不清楚」——那不是秘密，是你的底牌。
