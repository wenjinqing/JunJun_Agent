...
def _chat_ctx():
    token = current_chat_id.set("qq:999000111:group")
    yield
...
class TestFindUserId:
    def test_exact_match(self, monkeypatch):
        ...
        rows = [_Row("10000099", "鹤", "qq:999000111:group")]
        monkeypatch.setattr(db, "Messages", _fake_messages(rows))
        out = find_user_id.invoke({"nickname": "鹤"})
        assert "10000099" in out and "当前会话" in out

    def test_digit_passthrough(self):
        out = find_user_id.invoke({"nickname": "10000099"})
    ...
    def test_multiple_candidates(self, monkeypatch):
        import junjun_core.database as db
        rows = [_Row("111", "小白兔", "qq:999000111:group"),
                _Row("222", "大白兔", "qq:999000111:group"),
                _Row("333", "兔斯基", "qq:999000111:group")]
        monkeypatch.setattr(db, "Messages", _fake_messages(rows))
...
class TestMcpResolve:
    ...
    def test_resolve_digit_passthrough(self, _server):
        assert _server._resolve_user_id("qq", "10000099") == "10000099"

    def test_resolve_nickname(self, _server):
        assert _server._resolve_user_id("qq", "鹤") == "10000099"
    ...
    def test_penalty_accepts_nickname(self, _server, monkeypatch):
        ...
        assert added["uid"] == "10000099"
        assert "10000099" in out
