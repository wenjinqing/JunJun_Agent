...
_CLAIM_RULES: List[Dict] = [
    {
        ...
        # 「画出来」已移除：画画是高频话题，「等你画出来我看看」这类鼓励对方
        # 的表达全中（2026-08-06 审查实锤误拦面）；等下就发 加 (?<!你) 排除
        # 「你等下就发给他吧」这类指导性表达。
        "patterns": [r"画好[了啦]", r"画完[了啦]", r"在画了", r"正在画",
                     r"(?<!你)等(?:一下|一会儿|会儿|下)?就发"],
        "tools": {"ai_draw"},
    },
    {
        "name": "voice_sent",
    ...
    {
        "name": "message_sent",
        # 「已经发了」裸词太泛：转述第三方「他说通知已经发了你去看看」全中。
        # 收窄为带目标/方向的强声称（发给你/发过去/发出去）。
        "patterns": [r"消息发好", r"消息发过", r"消息发给你",
                     r"已经发(?:给你|过去|出去)"],
        "tools": {"send_message"},
    },
    {
    ...
    {
        "name": "novel_sent",
        # 「《X》抓取完成 / 发你了《X》 / txt 已发」是 pixiv 小说命令回执的专有
        # 措辞；LLM 输出它 = 模仿上文命令回执空口报发（2026-08-07 实锤：私聊
        # 裸链接根本没抓，5 秒就报出上一篇标题「发你了，《奴隶神明的最后一天》」）。
        # 命令路径的真实回执不经 LLM 输出，不受此规则影响。
        "patterns": [r"发你了[，,]?\s*《", r"《[^》]{1,40}》(?:已|已经)?发你",
                     r"抓取完成", r"txt 已发"],
        "tools": {"pixiv_download_novel"},
    },
]
