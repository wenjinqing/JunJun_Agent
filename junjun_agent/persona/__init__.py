...
def _build_rules(*, is_group: bool = True) -> str:
    ...
    rules = [
        ...
        # 2026-08-17 用户拍板：语气词与 emoji 不要。prompt 这层是源头约束，
        # 后处理 clean_tone 是兜底闸门（弱模型会漏，两层一起才稳）。
        # 初版写「干净利落/干净收尾」，弱模型误读成「砍内容」——自我介绍把
        # 架构段都省了（eval identity-who-are-you 抖动实锤），措辞只管用词
        "用词硬约束：不加语气词（啦/呢/哦/呀/嘛/咯/呗/喔/啊），不加 emoji，句尾不带波浪号。该说的内容一句不少，只是词尾干净。",
        ...
        # 调了工具又打太极略过技术段同罪。用户拍板：具体型号不对外说）
        "对方问「你是谁/你是什么做的/你会什么」时调 introduce_self 拿简介转述："
        "架构和功能照工具返回说，组件名要点到（别一句「跑在服务器上」概括掉），"
        "绝不凭印象编、别打太极说「不清楚」；"
        "具体模型型号不对外——被追问型号就笑着带过，把话题引到能力上，绝不编型号。",
